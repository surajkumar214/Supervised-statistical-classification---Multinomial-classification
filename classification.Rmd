...
sum(as.character(ablone.train$rings)!=as.character(predict(m1)))*100/dim(ablone.train)[1]
...
sum(as.character(ablone.test$rings)!=as.character(predict(step_both,ablone.test)))*100/dim(ablone.test)[1]

sum(as.character(ablone.train$rings)!=as.character(predict(step_both)))*100/dim(ablone.train)[1]
...
sum(as.character(ablone.test$rings)!=as.character(predict(m2,df)))*100/dim(ablone.test)[1]

#apply tree classification
#out of bag estimate, randomforest
ablone.raf <- randomForest(rings~., data = ablone.train, ntree = 200)
print(ablone.raf)
importance(ablone.raf)
varImpPlot(ablone.raf)
mean(as.character(ablone.train$rings)!=as.character(predict(ablone.raf)))
mean(as.character(ablone.test$rings)!=as.character(predict(ablone.raf,ablone.test)))

ablone.raf$confusion
  

t<- table(as.character(ablone.train$rings),as.character(predict(ablone.raf)))
round(prop.table(t,margin = 1),2)
...
train.set <- ablone.upd[ ind,]
test.set <- ablone.upd[-ind,]
...
#variable selection
step_both_car <- stepAIC(c1, direction="both")
mean(car.train$class!=predict(step_both_car))
mean(car.test$class!=predict(step_both_car,car.test))
...
mean(as.character(nursery.train$class)!=as.character(predict(step_both_nur)))
mean(as.character(nursery.test$class)!=as.character(predict(step_both_nur,nursery.test)))
...
mean(as.character(contra.train$contraceptive)!=as.character(predict(step_both_con)))
mean(as.character(contra.test$contraceptive)!=as.character(predict(step_both_con,contra.test)))

#apply randomforest
contra.train <- droplevels(contra.train )
contra.raf <- randomForest(contraceptive~., data = contra.train, ntree = 200)
print(contra.raf)
importance(contra.raf)
varImpPlot(contra.raf)

mean(as.character(contra.train$contraceptive)!=as.character(predict(contra.raf)))
mean(as.character(contra.test$contraceptive)!=as.character(predict(contra.raf,contra.test)))
...
#scaling
maxs <- apply(trainData[,c(19,20)], 2, max)
mins <- apply(trainData[,c(19,20)], 2, min)
...
#scaling
maxs <- apply(testData[,c(19,20)], 2, max)
mins <- apply(testData[,c(19,20)], 2, min)
